...
struct Weather {
    ...
    init(locationName: String?, temperature: Double?, description: String?, iconSrc: URL?, high: Double?, low: Double?, forecasts: [Weather]?) {
...
func getWeather(lat: Double, lon: Double, completion: @escaping (Weather?) -> Void) {
    if let apiKey = ProcessInfo.processInfo.environment["openweatherapikey"]{
                        ...
                        if let main = json["main"] as? [String: Any],
                           ...
                           let locationName = json["name"] as? String{
                            ...
                            let weather = Weather(locationName: locationName, temperature: temp, description: description, iconSrc:
                                                    URL(string:"https://openweathermap.org/img/wn/\(iconSrc)@2x.png"), high: high, low: low, forecasts: nil)
                            completion(weather)
                        } else {
                            completion(nil)
                        }
        ...
        print("Cannot find api key, aborting!")
        completion(nil)
        return
    }
}
